...
export default function NewStudyPage() {
  ...
  const [aiContext, setAiContext] = useState<LeadStudyContext | null>(null);
  ...
  useEffect(() => {
    ...
    async function load(leadId: string) {
      try {
        // 1. Dados básicos do lead.
        const leadRes = await fetch(`/api/leads/${leadId}`);
        const leadData = (await leadRes.json()) as { lead?: LeadLite };
        if (cancelled || !leadData.lead) {
        ...
        const mappedFromLead = mapCategory(lead.category);
        if (mappedFromLead) setCategory(mappedFromLead);
        setStudyTitle(`Estudo estratégico - ${lead.company || lead.name}`);

        // 2. Briefing interpretado pela IA (best-effort, não derruba o fluxo).
        const ctxRes = await fetch(`/api/leads/${leadId}/study-context`, {
          method: "POST",
        });
        const ctxData = (await ctxRes.json()) as { context?: LeadStudyContext };
        if (cancelled) return;
        if (ctxData.context) {
          const ctx = ctxData.context;
          setAiContext(ctx);
          if (ctx.suggested_title.trim()) setStudyTitle(ctx.suggested_title.trim());
          const mappedFromAi = mapCategory(ctx.category);
          if (mappedFromAi) setCategory(mappedFromAi);
        }
      } catch {
        // silently ignore: o fluxo manual continua disponível
      } finally {
        if (!cancelled) setContextLoading(false);
      }
    }

    void load(leadIdParam);
    return () => {
      cancelled = true;
    };
  }, [leadIdParam]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (aiContext) {
        // Pré-preenche o questionário com o que a IA inferiu do diagnóstico.
        Object.assign(baseAnswers, aiContext.prefilled_answers || {});
        // Alimenta o prompt de geração: context_summary entra como contexto setorial.
        if (aiContext.context_summary.trim()) {
          baseAnswers.__sector_context = {
            ...(typeof baseAnswers.__sector_context === "object" &&
            baseAnswers.__sector_context
              ? (baseAnswers.__sector_context as Record<string, unknown>)
              : {}),
            context_notes: aiContext.context_summary,
            suggested_questions: aiContext.open_questions || [],
          };
        }
      }
      ...
      if (leadContext || sectorCtx || aiContext) {
        ...
        if (aiContext) {
          genMeta.lead_context = {
            lead_id: leadContext?.id ?? leadIdParam,
            context_summary: aiContext.context_summary,
            open_questions: aiContext.open_questions || [],
            source: "diagnostic",
          };
          genMeta.sector_context = aiContext.context_summary;
        }
        if (sectorCtx) {
        ...
        {contextLoading && !aiContext && (
          ...
          >
            ...
            <div style={{ fontWeight: 700, color: "var(--navy)", fontSize: 13 }}>
              Lendo o diagnóstico do lead...
            </div>
          </div>
        )}

        {leadContext && aiContext && (
          <div
            style={{
              padding: "18px 20px",
              background: "rgba(82,225,231,0.08)",
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 800, color: "var(--navy)", fontSize: 14 }}>
                  Contexto importado do diagnóstico de {leadContext.name}
                </div>
                <div
                  style={{ fontSize: 11.5, color: "var(--ink-soft)", marginTop: 3 }}
                >
                  A IA leu o que o lead respondeu e pré-preencheu título, categoria e
                  parte do questionário. Revise e edite o que precisar.
                </div>
              </div>
              <button
                type="button"
            ...
            {aiContext.context_summary.trim() && (
              <div style={{ marginBottom: 14 }}>
                ...
                >
                  {summaryOpen ? "▾" : "▸"} O que entendemos da ideia
                </button>
                {summaryOpen && (
                  <div
                    style={{
                  ...
                  >
                    <MarkdownView md={aiContext.context_summary} />
                  </div>
                )}
              </div>
            )}

            {aiContext.open_questions.length > 0 && (
              <div>
                <div
                  className="overline"
                  style={{ marginBottom: 8, color: "var(--ink-soft)" }}
                >
                  O que falta cobrir no estudo
                </div>
                <ul
                  style={{
                    margin: 0,
                ...
                >
                  {aiContext.open_questions.map((q, i) => (
                    <li
                      key={i}
                      style={{
                        display: "flex",
                    ...
                    >
                      <span
                        style={{
                          ...
                          height: 16,
                          borderRadius: 5,
                          border: "2px solid rgba(82,225,231,0.6)",
                          flexShrink: 0,
                          marginTop: 1,
                        }}
                      />
